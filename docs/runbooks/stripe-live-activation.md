...
- [ ] 口座名義 (カタカナ、半角)
  - 個人口座でも OK (例: `サワイ コウキ`)
  - 屋号付き口座があれば優先 (例: `migrate-bot サワイ コウキ`)
...
corepack pnpm exec wrangler secret put STRIPE_SECRET_KEY --env=prod
# プロンプトで sk_live_... を貼り付け
```

### §4.3 Live mode Webhook 作成

1. https://dashboard.stripe.com/webhooks にアクセス (Live mode で)
2. **Add endpoint** をクリック
3. 入力:
   - **Endpoint URL**: `https://api.migrate-bot.dev/webhooks/stripe`
   - **Description**: `migrate-bot prod webhook`
   - **Events to send**:
     - `checkout.session.completed`
     - `checkout.session.expired`
     - `charge.refunded`
4. **Add endpoint** で保存
5. 作成された webhook 詳細ページで **Signing secret** (`whsec_...`) をコピー
6. Worker secret に登録:

```powershell
cd "C:\Users\FroGr\Desktop\Claude Code Projects\apps\api"
corepack pnpm exec wrangler secret put STRIPE_WEBHOOK_SECRET --env=prod
# プロンプトで whsec_... を貼り付け
```
...
deploy 完了後、API 側のログで Live mode key が反映されているか軽く確認 (起動時の log に key prefix が出る場合がある)。
...
- [ ] migrate-bot が起動 → 自分の GitHub repo で migration ジョブ実行
- [ ] PR まで動作確認
- [ ] Stripe Dashboard で transaction が "succeeded" になっているか確認
- [ ] support@migrate-bot.dev に注文確認メールが届くか確認
- [ ] 動作確認後、自分への refund 処理を test (`charge.refunded` webhook が動くか確認)
...
**Last updated**: 2026-05-12 (Karigo 住所確定により申請着手可能になったタイミング)
